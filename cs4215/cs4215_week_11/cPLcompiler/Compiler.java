...
public class Compiler {
	...
	public static CompilerResult compile(Expression exp, boolean opt,
			boolean verb) {
		...
		try {
			...
			INSTRUCTION[] ia = (INSTRUCTION[]) instructions
					.toArray(new INSTRUCTION[1]);

			if (verb)
				Compiler.displayInstructionArray(ia);

			return new CompilerResult(ia, divisionByZeroAddress,
					invalidRecordAccessAddress);
	...
	protected static void comp(Expression exp, INSTRUCTION toInsert,
		...
		if (exp instanceof Let) {
			...
			comp(new Application(new Fun(formals, ((Let) exp).body), arguments),
					toInsert, funVar);
		}
			...
			toCompileStack.push(new ToCompile(i, newIndexTable,
					((Try) exp).withExpression, "", toInsert));
		}

		else {

			if (exp instanceof Variable) {
				instructions.add(new LD(indexTable
						.access(((Variable) exp).varname)));
				...
				if (optimized) {
					...
					instructions.add(new CALL(
							((LookupApplication) exp).operands.size()));
				...
				instructions.add(new LDCI(indexTable
						.access(((Assignment) exp).leftHandSide)));
				...
				toCompileStack.push(new ToCompile(i, indexTable,
						((ThreadExpression) exp).expression, "",
						new ENDTHREAD()));
			} else if (exp instanceof Wait) {
				instructions.add(new WAIT(indexTable
						.access(((Wait) exp).varname)));
			} else if (exp instanceof Signal) {
				instructions.add(new SIGNAL(indexTable
						.access(((Signal) exp).varname)));
			}
